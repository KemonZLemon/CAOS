...
pthread_mutex_t mutex_on_global_total = PTHREAD_MUTEX_INITIALIZER;
...
#else
    pthread_mutex_lock( &mutex_on_global_total );
    /* ENTERING THIS THREAD'S CRITICAL SECTION */
    {
      total += i;  /* add i to the global total variable */
    }
    /* EXITING THIS THREAD'S CRITICAL SECTION */
    pthread_mutex_unlock( &mutex_on_global_total );
#endif
  }

  /* This code below is useless (does not change total), but it
   *  shows that we can have multiple critical sections on global total
   */
  pthread_mutex_lock( &mutex_on_global_total );
  /* ENTERING THIS THREAD'S CRITICAL SECTION */
  {
...
/*
 * JAVA:     synchronized ( mutexVariable ) {
 *             ...
 *             total += i;
 *           }
 */
